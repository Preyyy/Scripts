...
--- 15% CASH
    [1070] as HMO_15_PERCENT_Cash_Transaction,
    case
        when round([1004] * 0.15 + [1004], 2) = [1070] then 'TRUE'
        else 'FALSE'
    end as HMOCHECK15,
    cast(round([1004] * 0.15 + [1004], 2)as float) as cash15,
...
-- WHERE round([1004] * 0.05 + [1004], 2) != [1027] 
-- WHERE round([1004] * 0.10 + [1004], 2) != [1039]
-- WHERE round([1004] * 0.15 + [1004], 2) != [1070]
-- WHERE round([1004] * 0.12 + [1004], 2) != [1045]
-- WHERE round([1004] * 0.20 + [1004], 2) != [1066]

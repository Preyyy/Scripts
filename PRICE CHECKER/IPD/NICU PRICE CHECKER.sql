...
ItemDescription,
   ...
   -- NICU
    ...
    cast(round([1012] * 0.05 + [1012], 2)as float) as nicu5,
	CAST( round([1012] * 0.05 + [1012], 2)AS FLOAT) as nicu_iso5,
	CONCAT(
        'update iwitemprices set price = ''',
        CAST(round([1012] * 0.05 + [1012], 2, 2) AS FLOAT),
        '''',
        ' where FK_iwitems = ''',
        ItemID,
        '''',
        ' and FK_mscPriceSchemes in (''1024'')'
    ) AS SCRIPT_HM05,
    --- 10% NICU
    ...
    cast(round([1012] * 0.10 + [1012], 2)as float) as nicu10,
	CONCAT(
        'update iwitemprices set price = ''',
        CAST(round([1012] * 0.10 + [1012], 2, 2) AS FLOAT),
        '''',
        ' where FK_iwitems = ''',
        ItemID,
        '''',
        ' and FK_mscPriceSchemes in (''1040'')'
		) AS SCRIPT_HM10,
    --- 12% NICU
    ...
    cast(round([1012] * 0.12 + [1012], 2)as float) as nicu12,
	CONCAT(
        'update iwitemprices set price = ''',
        CAST(round([1012] * 0.12 + [1012], 2, 2) AS FLOAT),
        '''',
        ' where FK_iwitems = ''',
        ItemID,
        '''',
        ' and FK_mscPriceSchemes in (''1053'')'
    ) AS SCRIPT_HMO12,
    --- 15% NICU
    [1081] as HMO_12_PERCENT_NICU,
    case
        when round([1012] * 0.15 + [1012], 2) = [1081] then 'TRUE'
        else 'FALSE'
    end as HMOCHECK15,
    cast(round([1012] * 0.15 + [1012], 2)as float) as nicu15,
	CONCAT(
        'update iwitemprices set price = ''',
        CAST(round([1012] * 0.15 + [1012], 2, 2) AS FLOAT),
        '''',
        ' where FK_iwitems = ''',
        ItemID,
        '''',
        ' and FK_mscPriceSchemes in (''1081'')'
    ) AS SCRIPT_HMO15,
    --- 20% NICU
    ...
    cast(round([1012] * 0.20 + [1012], 2)as float) as nicu20
    FROM PivotData
    PIVOT (
      MAX(ItemPrice)
   ...
   -- where round([1012] * 0.05 + [1012], 2) != [1024]
   --  where round([1012] * 0.10 + [1012], 2) != [1040]
   -- where round([1012] * 0.12 + [1012], 2) != [1053]
  where  round([1012] * 0.15 + [1012], 2) != [1081]

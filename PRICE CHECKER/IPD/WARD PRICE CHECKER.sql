...
-- ward
    ...
    CAST(round([1009] * 0.05 + [1009], 2)AS FLOAT) as ward5,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1009] * 0.05 + [1009], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1021'')'
        ) AS SCRIPT_HM05,
    --- 10% WARD
    ...
    CAST(round([1009] * 0.10 + [1009], 2)AS FLOAT) as ward10,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1009] * 0.10 + [1009], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1030'')'
        ) AS SCRIPT_HM10,
    --- 12% WARD
    ...
    CAST(round([1009] * 0.12 + [1009], 2)AS FLOAT) as ward12,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1009] * 0.12 + [1009], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1050'')'
        ) AS SCRIPT_HM12,
    --- 15% WARD
    [1071] as HMO_15_PERCENT_Ward,
    case
        when round([1009] * 0.15 + [1009], 2) = [1071] then 'TRUE'
        else 'FALSE'
    end as HMOCHECK12,
    CAST(round([1009] * 0.15 + [1009], 2)AS FLOAT) as WARD15,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1009] * 0.15 + [1009], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1071'')'
        ) AS SCRIPT_HM15,
    --- 20% WARD
    ...
    CAST(round([1009] * 0.20 + [1009], 2)AS FLOAT) as ward20,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1009] * 0.20 + [1009], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1057'')'
        ) AS SCRIPT_HM20
FROM PivotData
PIVOT (
  MAX(ItemPrice)
...
--where round([1009] * 0.05 + [1009], 2) != [1021]
--where round([1009] * 0.10 + [1009], 2) != [1030]
-- where round([1009] * 0.12 + [1009], 2) != [1050]
-- where round([1009] * 0.15 + [1009], 2) != [1071]
 where round([1009] * 0.20 + [1009], 2) != [1057]

...
-- recovery
    ...
    CAST(round([1010] * 0.05 + [1010], 2)AS FLOAT) as recov5,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1010] * 0.05 + [1010], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1022'')'
        ) AS SCRIPT_HM05,
    --- 10% RECOVERY
    ...
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1010] * 0.10 + [1010], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1034'')'
        ) AS SCRIPT_HM10,
    --- 12% RECOVERY
    ...
    cast(round([1010] * 0.12 + [1010], 2)as float) as recov12,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1010] * 0.12 + [1010], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1051'')'
        ) AS SCRIPT_HM12,
    --- 15% RECOVERY
    [1075] as HMO_12_PERCENT_Recovery,
    case
        when round([1010] * 0.15 + [1010], 2) = [1075] then 'TRUE'
        else 'FALSE'
    end as HMOCHECK12,
    cast(round([1010] * 0.15 + [1010], 2)as float) as recov15,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1010] * 0.15 + [1010], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1075'')'
        ) AS SCRIPT_HM15,
    --- 20% RECOVERY
    ...
    cast(round([1010] * 0.20 + [1010], 2)as float) as recov20,
    ---SCRIPT FOR UPDATE
        CONCAT(
            'update iwitemprices set price = ''',
            CAST(round([1010] * 0.20 + [1010], 2, 2) AS FLOAT),
            '''',
            ' where FK_iwitems = ''',
            ItemID,
            '''',
            ' and FK_mscPriceSchemes in (''1061'')'
        ) AS SCRIPT_HM20
    FROM PivotData
    PIVOT (
      MAX(ItemPrice)
    ...
    -- where round([1010] * 0.05 + [1010], 2) != [1022]
	-- where round([1010] * 0.10 + [1010], 2) != [1034]
	-- where round([1010] * 0.12 + [1010], 2) != [1051]
	-- where round([1010] * 0.15 + [1010], 2) != [1075]
	-- where round([1010] * 0.20 + [1010], 2) != [1061]
